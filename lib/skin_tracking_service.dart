...
//         inputBuffer[0][y][x][0] = pixel.b / 255.0;  // B
//         inputBuffer[0][y][x][1] = pixel.g / 255.0;  // G
// inputBuffer[0][y][x][2] = pixel.r / 255.0;  // R
//       }
//     }
...
import 'package:image/image.dart' as img;
import 'package:tflite_flutter/tflite_flutter.dart';
import 'package:exif/exif.dart'; // Add this import for EXIF orientation
...
class SkinTrackingService {
  ...
  Future<String> predictImage(File image) async {
    ...
    try {
      ...
      final fixedImage = applyExifOrientation(decodedImage, exifData);
      
      print('Image dimensions: ${fixedImage.width}x${fixedImage.height}');
      ...
      // Convert to model input format
      final input = _convertToModelInput(resizedImage);
      print('Input buffer shape: ${input.length}-${input[0].length}-${input[0][0].length}-${input[0][0][0].length}');

      // Create output buffer (handle both quantized and float models)
      final outputTensor = _interpreter!.getOutputTensor(0);
      final isQuantized = outputTensor.type == TensorType.uint8;
      
      dynamic output;
      if (isQuantized) {
        output = List<List<int>>.filled(1, List<int>.filled(_labels.length, 0));
      } else {
        output = List<List<double>>.filled(1, List<double>.filled(_labels.length, 0.0));
      }
      ...
      dynamic result;
      if (isQuantized) {
        final dequantized = output[0].map((val) => val / 255.0).toList();
        print('Raw model output (quantized): ${output[0]}');
        print('Dequantized output: $dequantized');
        result = _interpretResult(dequantized);
      } else {
        print('Raw model output: ${output[0]}');
        result = _interpretResult(output[0]);
      }
  ...
  Future<String> testPrediction() async {
    try {
      final testFile = File('assets/test_skin.jpg');
      if (!await testFile.exists()) {
        throw Exception('Test image not found at assets/test_skin.jpg');
      }
      return await predictImage(testFile);
    } catch (e) {
  ...
  List<List<List<List<double>>>> _convertToModelInput(img.Image image) {
    ...
    // Fill the buffer with normalized pixel values (BGR order)
    for (var y = 0; y < _inputSize; y++) {
      for (var x = 0; x < _inputSize; x++) {
        final pixel = image.getPixel(x, y);
        inputBuffer[0][y][x][0] = pixel.b / 255.0;  // B
  ...
  /// Applies EXIF orientation to image
  img.Image applyExifOrientation(img.Image image, Map<String, IfdTag> exifData) {
    if (!exifData.containsKey('Image Orientation')) return image;
  ...
  readExifFromBytes(Uint8List imageBytes) {}
}